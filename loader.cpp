...
loader::loader()
{
    char *temp[]= {"id","prc","name","dicountstatus","dst","time","status"};
    string* test=new string("id=1:prc=10:name=nikolau:dicountstatus=false:dst=moskow:time=1120:status=available;id=1:prc=100:name=david:dicountstatus=true:dst=tver:time=1535:status=available;");
    tickets=test;
    int i=0;
...
void testPrint(string test)
{
    cout<<test;
}

loader::pPair loader::findParamInParamPairs(string* str,char* params[],int qty)
{
    int i;
    char* strPar;
    pPair parPair;
...
char* loader::findSymbolFromPoint(char* start,char symbol)
{
    while(*start!='\0')
    {
        if(*start==symbol)
...
vector<string*> loader::divideTicketString(string str,char symbol)
{
...
string* loader::copyStringPointToPoint(char* s,char* e)
{
...
void loader::setParamFields(vector<string*> strVector)
{
    paramFiels=strVector;
}
void loader::setParamsMethod(vector<void (*)(string)> methods)
{
    paramMethod=methods;
}
void loader::setAnalizeData(string* data)
{
    tickets=data;
}
